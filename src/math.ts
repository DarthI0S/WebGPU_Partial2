...
export const vec3 = {
  ...
  barycentric(A: Vec3, B: Vec3, C: Vec3, P: Vec3): Vec3{
    const v0 = this.sub(B, A);//a->b (b-a)
    const v1 = this.sub(C, A); //a->c (c-a)
    const v2 = this.sub(P, A); //a->p (p-a)

    const d00 = this.dot(v0, v0);
    const d01 = this.dot(v0, v1);
    const d11 = this.dot(v1, v1);
    const d20 = this.dot(v2, v0);
    const d21 = this.dot(v2, v1);
...
rotationAxis(axis: Vec3, angle: number): Mat4 {
